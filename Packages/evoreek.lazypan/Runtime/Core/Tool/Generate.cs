...
namespace LazyPan {
    ...
    public class Generate : EditorWindow {
        ...
        public static void GenerateBehaviourBySign(string sign, bool isTemplate) {
            /*读取配置*/
            ReadCSV.Instance.Read("BehaviourGenerate", out string content, out string[] lines);
            LogUtil.Log(content);
                ...
                if (i > 2) {
                    ...
                    if (strs[0] == sign) {
                        ...
                        if (curMethodCode == "Namespace") {
                            ...
                            string methodDescription = strs[4];
                            string methodKey = string.Concat(method, "|", methodDescription);
                            if (customer.ContainsKey(methodKey)) {
                                //自定义存储了该方法
                                customer[methodKey].Add(curMethodCode);
                            } else {
                                List<string> frags = new List<string>();
                        ...
                        if (i + 1 == lines.Length || (i + 1 != lines.Length && lines[i + 1].Split(',')[0] != curBehaviourSign)) {
                            ...
                            log = Regex.Replace(log, "#命名空间#", curNamespace);
                            log = Regex.Replace(log, "#行为类型#", curBehaviourType);
                            log = Regex.Replace(log, "#行为标识#",
                                isTemplate ? string.Concat(curBehaviourSign, "_Template") : curBehaviourSign);
                            ...
                            if (isTemplate) {
                                createPath =
                                    $"Assets/LazyPan/Scripts/GamePlay/Behaviour/Template/Behaviour_{type}_{string.Concat(curBehaviourSign, "_Template")}.cs";
                            } else {
                                createPath =
                                    $"Assets/LazyPan/Scripts/GamePlay/Behaviour/Behaviour_{type}_{curBehaviourSign}.cs";
                            }
